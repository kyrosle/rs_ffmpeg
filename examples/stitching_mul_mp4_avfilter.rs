...
use rs_ffmpeg::{
  ...
  avutil::{
    av_inv_q, av_rescale_q, get_sample_fmt_name, ra, AVChannelLayout,
    AVDictionary, AVFrame,
  },
  error::RsmpegError,
  ffi::{self, AVMEDIA_TYPE_AUDIO, AVMEDIA_TYPE_VIDEO},
};
...
fn open_input_file(
  filename: &CStr,
) -> Result<(Vec<Option<AVCodecContext>>, AVFormatContextInput)> {
...
fn open_output_file(
  ...
  for (i, dec_ctx) in dec_ctx.into_iter().enumerate() {
    ...
    if ofmt_ctx.oformat().flags & ffi::AVFMT_GLOBALHEADER as i32 != 0 {
      enc_ctx
        .set_flags(enc_ctx.flags | ffi::AV_CODEC_FLAG_GLOBAL_HEADER as i32);
    }
...
fn init_filter<'graph>(
  ...
  if dec_ctx.codec_type == AVMEDIA_TYPE_AUDIO {
    ...
    let (_inputs, _outputs) =
      filter_graph.parse_ptr(filter_spec, Some(inputs), Some(outputs))?;
  } else if dec_ctx.codec_type == AVMEDIA_TYPE_VIDEO {
    let fade = AVFilter::get_by_name(cstr!("fade"))?;
    let args = &CString::new("type=out:st=2:d=1").unwrap();
    let mut fade_ctx =
      filter_graph.create_filter_context(&fade, cstr!("fade"), Some(args))?;
    buffersrc_ctx.link(0, &mut fade_ctx, 0)?;
    fade_ctx.link(0, &mut buffersink_ctx, 0)?;
  }

  filter_graph.config()?;
  // println!("-- Filter graph dump \n{}", filter_graph.dump()?);
...
fn init_filters(
  ...
  for (filter_graph, stream_context) in
    filter_graphs.iter_mut().zip(stream_contexts.into_iter())
  {
...
fn handle_packet(
  ...
  ofmt_ctx: &mut AVFormatContextOutput,
  filter_ctx: &mut [Option<FilteringContext>],
  pts_offset: &mut i64,
) -> Result<i64> {
  let mut pts_offset_loop = 0;
  loop {
    ...
    }) = filter_ctx[in_stream_index].as_mut()
    {
      ...
      loop {
        ...
        pts_offset_loop = frame.best_effort_timestamp;
...
pub fn transcode(
  ...
  let (dec_ctx_1, mut ifmt_ctx_1) = open_input_file(input_file_1)?;
  let (dec_ctx_2, mut ifmt_ctx_2) = open_input_file(input_file_2)?;
  let (stream_ctx, mut ofmt_ctx) = open_output_file(
    output_file,
    dec_ctx_1.into_iter().chain(dec_ctx_2.into_iter()).collect(),
    dict,
  )?;
...
fn main() -> Result<()> {
  // unsafe {av_log_set_level(ffi::AV_LOG_INFO as i32)};
  std::fs::create_dir_all("examples/output/stitching_mul_mp4_avfilter/")
    .unwrap();
  transcode(
    cstr!("examples/assets/test.mp4"),
    cstr!("examples/assets/bear.mp4"),
    cstr!("examples/output/stitching_mul_mp4_avfilter/stitching_mp4.mp4"),
    &mut None,
  )?;
  Ok(())
}
