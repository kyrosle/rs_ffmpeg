...
fn main() {
  let filepath: CString = CString::new("./examples/bear.mp4").unwrap();
  ...
  if unsafe { ffi::avformat_find_stream_info(format_context, ptr::null_mut()) }
    < 0
  {
    ...
    .enumerate()
  {
    ...
    let local_codec = unsafe {
      ffi::avcodec_find_decoder(local_codec_params.codec_id).as_ref()
    }
  ...
  let codec_context =
    unsafe { ffi::avcodec_alloc_context3(codec_ptr).as_mut() }
      .expect("failed to allocated memory for AVCodecContext");

  if unsafe {
    ffi::avcodec_parameters_to_context(codec_context, codec_parameters_ptr)
  } < 0
  {
    panic!("failed to copy codec params to codec context");
  }

  if unsafe { ffi::avcodec_open2(codec_context, codec_ptr, ptr::null_mut()) }
    < 0
  {
