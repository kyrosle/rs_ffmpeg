...
pub type ReadPacketCallback =
  Box<dyn FnMut(&mut Vec<u8>, &mut [u8]) -> i32 + Send + 'static>;
pub type WritePacketCallback =
  Box<dyn FnMut(&mut Vec<u8>, &[u8]) -> i32 + Send + 'static>;
pub type SeekCallback =
  Box<dyn FnMut(&mut Vec<u8>, i64, i32) -> i64 + Send + 'static>;
