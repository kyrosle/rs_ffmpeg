...
impl AVFormatContextInput {
  ...
  pub fn open(
    ...
    let mut context =
      unsafe { Self::from_raw(NonNull::new(input_format_context).unwrap()) };

    unsafe {
      ffi::avformat_find_stream_info(context.as_mut_ptr(), ptr::null_mut())
    }
    .upgrade()
  ...
  pub fn from_io_context(mut io_context: AVIOContextContainer) -> Result<Self> {
    let input_format_context = {
      ...
      let input_format_context =
        unsafe { ffi::avformat_alloc_context() }.upgrade().unwrap();
      unsafe {
    ...
    let mut input_format_context =
      unsafe { Self::from_raw(input_format_context) };
    input_format_context.io_context = Some(io_context);

    unsafe {
      ffi::avformat_find_stream_info(
...
impl<'stream> AVFormatContextInput {
  ...
  pub fn streams(&'stream self) -> &'stream [AVStreamRef<'stream>] {
    ...
    let streams = self.streams as *const *const ffi::AVStream
      as *const AVStreamRef<'stream>;
    // u32 to usize, safe
    ...
    #[cfg(debug_assertions)]
    {
      let arr = unsafe {
        std::slice::from_raw_parts(
          self.streams as *const *const ffi::AVStream,
          len,
        )
      };
      for ptr in arr {
  ...
  pub fn iformat(&'stream self) -> AVInputFormatRef<'stream> {
    ...
    unsafe {
      AVInputFormatRef::from_raw(NonNull::new(self.iformat as *mut _).unwrap())
    }
  }
...
impl AVFormatContextOutput {
  ...
  pub fn create(
    ...
    if output_format_context.oformat().flags & ffi::AVFMT_NOFILE as i32 == 0 {
      ...
      let mut io_context = match io_context {
        Some(x) => x,
        None => AVIOContextContainer::Url(AVIOContextURL::open(
          filename,
          ffi::AVIO_FLAG_WRITE,
        )?),
      };
  ...
  pub fn write_header(
    ...
    let result = unsafe {
      ffi::avformat_write_header(self.as_mut_ptr(), &mut dict_ptr as _)
    };
  ...
  pub fn interleaved_write_frame(
    ...
    unsafe {
      ffi::av_interleaved_write_frame(self.as_mut_ptr(), packet.as_mut_ptr())
    }
    .upgrade()
...
impl<'stream> AVFormatContextOutput {
  ...
  pub fn oformat(&self) -> AVOutputFormatRef<'static> {
    ...
    unsafe {
      AVOutputFormatRef::from_raw(NonNull::new(self.oformat as *mut _).unwrap())
    }
  }
  ...
  pub fn new_stream(&'stream mut self) -> AVStreamMut<'stream> {
    ...
    let new_stream =
      unsafe { ffi::avformat_new_stream(self.as_mut_ptr(), ptr::null()) }
        .upgrade()
        .unwrap();
...
impl<'stream> AVStream {
  ...
  pub fn codecpar(&'stream self) -> AVCodecParametersRef<'stream> {
    ...
    unsafe {
      AVCodecParametersRef::from_raw(NonNull::new(self.codecpar).unwrap())
    }
  }
  ...
  pub fn codecpar_mut(&'stream mut self) -> AVCodecParametersMut<'stream> {
    unsafe {
      AVCodecParametersMut::from_raw(NonNull::new(self.codecpar).unwrap())
    }
  }
  ...
  pub fn set_metadata(&mut self, dict: Option<AVDictionary>) {
    // Drop the old_dict
    let _ =
      NonNull::new(self.metadata).map(|x| unsafe { AVDictionary::from_raw(x) });
