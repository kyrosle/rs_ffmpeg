...
use crate::{
  ...
  avutil::{
    AVChannelLayoutRef, AVDictionary, AVFrame, AVPixelFormat, AVRational,
  },
...
impl<'codec> AVCodec {
  ...
  fn probe_len<T>(mut ptr: *const T, tail: T) -> usize {
    for len in 0.. {
      if unsafe {
        libc::memcmp(ptr as _, &tail as *const _ as _, mem::size_of::<T>())
      } == 0
      {
        return len;
      }
...
impl AVCodecContext {
  ...
  pub fn open(
    ...
    if let Some(mut dict) = dict {
      ...
      unsafe {
        ffi::avcodec_open2(self.as_mut_ptr(), ptr::null_mut(), ptr::null_mut())
      }
      .upgrade()
      .map_err(RsmpegError::CodecOpenError)?;
      Ok(None)
  ...
  pub fn send_packet(&mut self, packet: Option<&AVPacket>) -> Result<()> {
    ...
    match unsafe { ffi::avcodec_send_packet(self.as_mut_ptr(), packet_ptr) }
      .upgrade()
    {
      Ok(_) => Ok(()),
  ...
  pub fn receive_frame(&mut self) -> Result<AVFrame> {
    ...
    match unsafe {
      ffi::avcodec_receive_frame(self.as_mut_ptr(), frame.as_mut_ptr())
    }
    .upgrade()
    {
      Ok(_) => Ok(frame),
  ...
  pub fn send_frame(&mut self, frame: Option<&AVFrame>) -> Result<()> {
    ...
    match unsafe { ffi::avcodec_send_frame(self.as_mut_ptr(), frame_ptr) }
      .upgrade()
    {
      Ok(_) => Ok(()),
  ...
  pub fn receive_packet(&mut self) -> Result<AVPacket> {
    ...
    match unsafe {
      ffi::avcodec_receive_packet(self.as_mut_ptr(), packet.as_mut_ptr())
    }
    .upgrade()
    {
      Ok(_) => Ok(packet),
  ...
  pub fn apply_codecpar(&mut self, codecpar: &AVCodecParameters) -> Result<()> {
    unsafe {
      ffi::avcodec_parameters_to_context(self.as_mut_ptr(), codecpar.as_ptr())
    }
    .upgrade()
