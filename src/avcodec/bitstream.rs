...
impl AVBSFContext {
  ...
  pub fn par_out(&self) -> AVCodecParametersRef<'_> {
    unsafe {
      AVCodecParametersRef::from_raw(NonNull::new(self.par_out).unwrap())
    }
  }
}
...
impl AVBSFContextUninit {
  ...
  pub fn filter(&self) -> AVBitStreamFilterRef {
    unsafe {
      AVBitStreamFilterRef::from_raw(
        NonNull::new(self.filter as *mut _).unwrap(),
      )
    }
  }
  ...
  pub fn set_par_in(&mut self, source_params: &AVCodecParameters) {
    unsafe {
      ffi::avcodec_parameters_copy(self.par_in, source_params.as_ptr())
    }
    .upgrade()
    .unwrap();
  }
