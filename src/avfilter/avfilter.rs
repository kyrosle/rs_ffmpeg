...
impl AVFilterContext {
  ...
  pub fn buffersrc_add_frame(
    ...
    unsafe {
      ffi::av_buffersrc_add_frame_flags(self.as_mut_ptr(), frame_ptr, flags)
    }
    .upgrade()
  ...
  pub fn buffersink_get_frame(
    ...
    let flags = flags.unwrap_or(0);
    match unsafe {
      ffi::av_buffersink_get_frame_flags(
        self.as_mut_ptr(),
        frame.as_mut_ptr(),
        flags,
      )
  ...
  pub fn get_ch_layout(&self) -> AVChannelLayout {
    ...
    unsafe {
      ffi::av_buffersink_get_ch_layout(self.as_ptr(), ch_layout.as_mut_ptr())
    }
    .upgrade()
    .unwrap();
...
impl AVFilterInOut {
  ...
  pub fn new(
    ...
    let mut inout_ptr =
      unsafe { ffi::avfilter_inout_alloc() }.upgrade().unwrap();
...
impl AVFilterGraph {
  ...
  pub fn new() -> Self {
    let filter_graph =
      unsafe { ffi::avfilter_graph_alloc() }.upgrade().unwrap();
  ...
  pub fn config(&self) -> Result<()> {
    ...
    unsafe {
      ffi::avfilter_graph_config(self.as_ptr() as *mut _, ptr::null_mut())
    }
    .upgrade()?;
    Ok(())
  }
