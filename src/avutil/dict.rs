...
impl AVDictionary {
  ...
  pub fn new(key: &CStr, value: &CStr, flags: u32) -> Self {
    ...
    unsafe {
      ffi::av_dict_set(&mut dict, key.as_ptr(), value.as_ptr(), flags as i32)
    }
    .upgrade()
    .unwrap();
  ...
  pub fn new_int(key: &CStr, value: i64, flags: u32) -> Self {
    let mut dict = ptr::null_mut();
    unsafe {
      ffi::av_dict_set_int(&mut dict, key.as_ptr(), value, flags as i32)
    }
    .upgrade()
    .unwrap();
    unsafe { Self::from_raw(NonNull::new(dict).unwrap()) }
...
impl<'dict> AVDictionary {
  ...
  pub fn get(
    ...
    unsafe {
      ffi::av_dict_get(self.as_ptr(), key.as_ptr(), prev_ptr, flags as i32)
    }
    .upgrade()
...
mod test {
  ...
  fn set() {
    ...
    let _ = AVDictionary::new(cstr!("bob"), cstr!("alice"), 0).set(
      cstr!("bob"),
      cstr!("alice"),
      0,
    );
  ...
  fn get() {
    ...
    let dict = AVDictionary::new(cstr!("foo"), cstr!("bar"), 0).set(
      cstr!("bob"),
      cstr!("alice"),
      0,
    );
  ...
  #[test]
  fn copy() {
    let dicta = AVDictionary::new(cstr!("a"), cstr!("b"), 0).set(
      cstr!("c"),
      cstr!("d"),
      0,
    );
