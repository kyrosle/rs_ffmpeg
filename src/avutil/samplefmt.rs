...
impl AVSamples {
  ...
  pub fn new(
    ...
    let mut audio_data =
      vec![ptr::null_mut(); nb_planes as usize].into_boxed_slice();
    let mut linesize = 0;
    ...
    let mut samples =
      unsafe { AVSamples::from_raw(NonNull::new(buffer).unwrap()) };
