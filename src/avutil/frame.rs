use crate::{
  avutil::{
    av_image_fill_arrays, AVChannelLayoutRef, AVImage, AVMotionVector,
    AVPixelFormat,
  },
  error::*,
  ffi,
...
impl AVFrame {
  ...
  pub fn is_writable(&self) -> Result<bool> {
    match unsafe { ffi::av_frame_is_writable(self.as_ptr() as *mut _) }
      .upgrade()
    {
      Ok(1) => Ok(true),
      Ok(0) => Ok(false),
...
impl<'frame> AVFrame {
  pub fn get_side_data(
    ...
    unsafe { ffi::av_frame_get_side_data(self.as_ptr(), side_data_type) }
      .upgrade()
      .map(|side_data_ptr| unsafe {
        AVFrameSideDataRef::from_raw(side_data_ptr)
      })
  }
}
