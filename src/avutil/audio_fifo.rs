...
impl AVAudioFifo {
  ...
  pub fn new(
    ...
    let fifo =
      unsafe { ffi::av_audio_fifo_alloc(sample_fmt, channels, nb_samples) }
        .upgrade()
        .unwrap();
  ...
  pub unsafe fn write(
    ...
    let ret = unsafe {
      ffi::av_audio_fifo_write(self.as_mut_ptr(), data as _, nb_samples)
    }
    .upgrade()?;
  ...
  pub unsafe fn peek(
    ...
    let ret = unsafe {
      ffi::av_audio_fifo_peek(self.as_mut_ptr(), data as _, nb_samples)
    }
    .upgrade()?;
    Ok(ret)
  }
  ...
  pub unsafe fn read(
    ...
    let ret = unsafe {
      ffi::av_audio_fifo_read(self.as_mut_ptr(), data as _, nb_samples)
    }
    .upgrade()?;
    Ok(ret)
  }
