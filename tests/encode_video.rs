...
fn encode_video(codec_name: &CStr, file_name: &str) -> Result<()> {
  let encoder = AVCodec::find_encoder_by_name(codec_name)
  ...
  if encoder.id == ffi::AV_CODEC_ID_H264 {
    unsafe {
      opt_set(encode_context.priv_data, cstr!("preset"), cstr!("slow"), 0)
    }
  ...
  let file = File::create(file_name)
    .with_context(|| anyhow!("Could not open: {}", file_name))?;
