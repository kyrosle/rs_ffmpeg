...
use rs_ffmpeg::ffi::AV_INPUT_BUFFER_PADDING_SIZE;
use rs_ffmpeg::{
  avcodec::{AVCodecContext, AVCodecParserContext, AVPacket},
  avformat::AVFormatContextInput,
  avutil::{
    get_bytes_per_sample, get_packed_sample_fmt, get_sample_fmt_name,
    sample_fmt_is_planar, AVFrame, AVSampleFormat,
  },
  error::RsmpegError,
  ffi,
};
...
fn frame_save(
  ...
  let nb_samples: usize =
    frame.nb_samples.try_into().context("nb_samples overflow")?;
  // ATTENTION: This is only valid for planar sample formats.
  for i in 0..nb_samples {
    for channel in 0..channels {
      let data = unsafe {
        from_raw_parts(frame.data[channel].add(data_size * i), data_size)
      };
...
fn decode_audio(audio_path: &str, out_file_path: &str) -> Result<()> {
  ...
  let (decoder, mut decode_context) = {
    ...
    let mut input_format_context =
      AVFormatContextInput::open(&audio_path, None, &mut None)
  ...
  let mut inbuf =
    [0u8; AUDIO_INBUF_SIZE + AV_INPUT_BUFFER_PADDING_SIZE as usize];

  let mut audio_file = File::open(audio_path)
    .with_context(|| anyhow!("Could not open {}", audio_path))?;
  fs::create_dir_all(Path::new(out_file_path).parent().unwrap()).unwrap();
  let out_file =
    File::create(out_file_path).context("Open out file failed.")?;

  let mut parser_context =
    AVCodecParserContext::init(decoder.id).context("Parser not found")?;
  ...
  if sample_fmt_is_planar(sample_fmt) {
    ...
    sample_fmt = get_packed_sample_fmt(sample_fmt)
      .context("Cannot get packed sample fmt")?;
  }

  let fmt =
    get_format_from_sample_fmt(sample_fmt).context("Unsupported sample fmt")?;

  println!("Play the output audio file with the command:");
  println!(
    "ffplay -f {} -ac {} -ar {} {}",
    fmt,
    decode_context.ch_layout.nb_channels,
