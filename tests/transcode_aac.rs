...
fn open_input_file(
  input_file: &CStr,
) -> Result<(AVFormatContextInput, AVCodecContext, usize)> {
  let input_format_context =
    AVFormatContextInput::open(input_file, None, &mut None)
...
fn open_output_file(
  ...
  let mut output_format_context =
    AVFormatContextOutput::create(output_file, None)
  ...
  encode_context.set_ch_layout(
    AVChannelLayout::from_nb_channels(OUTPUT_CHANNELS).into_inner(),
  );
...
fn load_encode_and_write(
  ...
  if unsafe { fifo.read(frame.data_mut().as_mut_ptr(), frame_size)? }
    < frame_size
  {
...
fn transcode_aac(input_file: &CStr, output_file: &CStr) -> Result<()> {
  ...
  let mut resample_context =
    init_resampler(&mut decode_context, &mut encode_context)?;
